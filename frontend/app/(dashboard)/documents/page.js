...
export default function DocumentsPage() {
    ...
    useEffect(() => {
        if (highlightId && documents.length > 0) {
            const doc = documents.find(d => d._id === highlightId);
                        ...
                        {filteredDocs.map(doc => (
                            <div
                                ...
                                className={`
                                    ...
                                    ${highlightId === doc._id ? 'ring-2 ring-primary-500' : ''}
                                `}
                            >
                                ...
                                <p className="text-sm text-slate-500">
                                    {doc.chunkCount || 0} chunks • {(doc.size / 1024).toFixed(1)} KB
                                </p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-2">
                        {filteredDocs.map(doc => (
                            <div
                                ...
                                onClick={() => setSelectedDocument(doc)}
                                className={`
                                    flex items-center gap-4 p-4 bg-slate-900 border border-slate-800 rounded-xl
