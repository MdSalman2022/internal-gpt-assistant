        ...
        <div className="flex flex-col h-full bg-transparent p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex items-center gap-4">
                    <div className="max-w-xs relative">
                        ...
                        <input
                            ...
                            className="w-full bg-secondary/50 border border-border/50 rounded-lg pl-10 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                        />
                    </div>
                    <div className="bg-secondary/50 px-4 py-2 rounded-lg border border-border/50">
                        <span className="text-muted-foreground text-sm">Total: </span>
                        <span className="text-foreground font-mono font-semibold">{users.length}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-card/50 border border-border/50 rounded-2xl overflow-hidden shadow-xl">
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b border-border/50 text-xs uppercase tracking-wider text-muted-foreground bg-secondary/30">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                                                    {u.name}
                                                    {u._id === currentUser.id && <span className="ml-2 text-xs text-muted-foreground">(You)</span>}
                                                </p>
                                    ...
                                    <td className="px-6 py-4 text-right" onClick={e => e.stopPropagation()}>
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                ...
                                                className="p-2 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg transition-colors text-xs font-medium border border-transparent"
                                                title="Change Role"
                                            >
                                                Edit Role
                                            </button>
                                            ...
                                            {u._id !== currentUser.id && (
                                                <button
                                                    ...
                                                    className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors border border-transparent"
                                                    title="Delete User"
                                                >
            ...
            {isEditModalOpen && selectedUser && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsEditModalOpen(false)}>
                    <div
                        className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-scale-in"
                        onClick={e => e.stopPropagation()}
                    >
                        <div className="p-6 border-b border-border">
                            <h3 className="text-xl font-semibold text-foreground">Edit User Role</h3>
                            <p className="text-muted-foreground text-sm mt-1">
                                Change access level for <span className="text-foreground font-medium">{selectedUser.name}</span>
                            </p>
                        </div>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center
                                            ${selectedUser.role === role ? 'border-primary' : 'border-border group-hover:border-primary/50'}
                                        `}>
                                            {selectedUser.role === role && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
                                        </div>
                                        <div>
                                            <span className={`block font-medium ${selectedUser.role === role ? 'text-primary' : 'text-foreground'}`}>
