...
import { 
    Key, Plus, Trash2, Shield, Pencil, Save,
    AlertCircle, Loader2, Sparkles, Bot, Brain, Zap
} from 'lucide-react';
import toast from 'react-hot-toast';

export default function APICredentialsSettings({ onUpdate }) {
    const { user } = useAuth();
    const [credentials, setCredentials] = useState([]);
    const [serverCredentials, setServerCredentials] = useState([]); // Source of truth
    ...
    const loadCredentials = async () => {
        try {
            ...
            if (data.success) {
                ...
                const sorted = data.credentials.sort((a, b) => {
    ...
    const handleToggleActive = (id, currentStatus, provider) => {
        const newStatus = !currentStatus;
        
        setCredentials(prev => {
            const next = prev.map(cred => {
                // 1. If this is the one we clicked, set its new status
                if (cred._id === id) {
                    return { ...cred, isActive: newStatus };
                }
                
                // 2. If we are enabling a key, disable all others (Global Exclusivity)
                if (newStatus) {
    ...
    useEffect(() => {
        if (credentials.length > 0 && serverCredentials.length > 0) {
            checkForChanges(credentials);
        }
    }, [credentials, serverCredentials]);

    const checkForChanges = (currentCreds) => {
        // Simple comparison: check if any isActive status differs from server credentials
        const changed = currentCreds.some(c => {
            const server = serverCredentials.find(s => s._id === c._id);
            return server && server.isActive !== c.isActive;
        });
        setHasChanges(changed);
    };

    const handleSaveChanges = async () => {
        setProcessingId('save-changes');
        
        // Find changed items
        const updates = credentials.filter(c => {
            const server = serverCredentials.find(s => s._id === c._id);
            return server && server.isActive !== c.isActive;
        }).map(c => ({ id: c._id, status: c.isActive }));
        ...
        try {
            ...
            toast.success('Credential preferences saved');
            
            // Reload from server to get actual state (backend enforces mutual exclusivity)
                ...
                <div className="flex items-center gap-3">
                    {hasChanges && (
                        <button 
                            ...
                            disabled={processingId === 'save-changes'}
                            className="flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-all shadow-sm font-medium text-sm animate-in fade-in slide-in-from-right-4"
                        >
                            {processingId === 'save-changes' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                            Save Changes
                        </button>
                    )}
                    <button 
            ...
            <div className="divide-y divide-border">
                {credentials.length === 0 ? (
                    ...
                    credentials.map((cred) => (
                        <div key={cred._id}>
                            <div className={`p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-muted/30 transition-colors ${!cred.isActive ? 'opacity-60 bg-muted/10' : ''}`}>
